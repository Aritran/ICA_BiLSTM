...
corpora, sentences = read_for_word2vec.read_files(paths)
...
model = Word2Vec([s.split() for s in sentences], sg=1, size=100, window=8, min_count=0, workers=multiprocessing.cpu_count())
    # ?????  ??????
for i in range(100):
